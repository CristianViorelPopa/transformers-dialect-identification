...
def main():
  ...
  # tokenizer = BertTokenizer.from_pretrained("models/ro-trans-cased_v1",
  #                                           do_lower_case=False)
  examples = []
  ...
  # examples = []
  # for features in utils.load_json(args.preprocessed_data_file):
  #   example = tokenizer.convert_tokens_to_ids(features['tokens'])
  #   if len(example) <= args.max_sequence_length:
  #     examples.append(example)

  print("Building BERT model...")
  extractor = AttnMapExtractor(
      os.path.join(args.bert_dir, "config.json"),
      os.path.join(args.bert_dir, "ro_trans_uncased_v2.ckpt"),
